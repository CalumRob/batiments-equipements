...
o <- data.table(id = "o1", lon = -1.6778, lat = 48.1173)   # Rennes centre
d <- data.table(
  id = paste0("d", 1:6),
  lon = c(-1.6778, -1.6294, -1.5980, -1.7090, -1.6493, -1.5550),
  lat = c(48.1173, 48.1119, 48.1329, 48.1025, 48.0861, 48.0972))

show <- function(tt, label) {
  cat("--", label, "--\n")
  if (is.null(tt)) { cat("NULL\n"); return(invisible()) }
  x <- as.data.table(tt)
  print(x)
}

for (dep in list(as.POSIXct("2026-08-26 05:00:00", tz = "UTC"),
                 as.POSIXct("2026-09-15 06:00:00", tz = "UTC"))) {
  cat("\n########## departure", format(dep, "%Y-%m-%dT%H:%M%z"), "##########\n")
  tr <- tryCatch(r5r::travel_time_matrix(
    ...
    error = function(e) { cat("TRANSIT ERR:", conditionMessage(e), "\n"); NULL })
  show(tr, "direct r5r TRANSIT")
  wk <- tryCatch(r5r::travel_time_matrix(
    net, origins = o, destinations = d, mode = "WALK",
    max_trip_duration = 20, verbose = FALSE),
    error = function(e) { cat("WALK ERR:", conditionMessage(e), "\n"); NULL })
  show(wk, "direct r5r WALK")
}
r5r::stop_r5(net)
