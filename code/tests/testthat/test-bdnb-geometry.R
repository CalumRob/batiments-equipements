...
test_that("fallback precedence is construction then real group centroid", {
  ...
  expect_equal(got[["geometry_source"]][1:3],
               c("geom_cstr", "geom_groupe", NA_character_))
  expect_equal(got[["x_2154"]][2], 21) # address rejected; group centroid used
  expect_true(is.na(got[["x_2154"]][3]))
})

test_that("fictive geometry is explicit and can be dropped", {
  got <- bdnb_resolve_geometry(geometry_fixture(), min_fiabilite = 21)
  expect_equal(got[["geometry_source"]][4:5], c("fictive", NA_character_))
  expect_true(got[["geometry_resolved"]][4])
  expect_false(got[["geometry_resolved"]][5])
  expect_equal(sum(!is.na(got[["geometry_source"]]) &
                   got[["geometry_source"]] != "fictive"), 2)
})
